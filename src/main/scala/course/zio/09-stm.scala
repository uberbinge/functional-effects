...
object StmQueue extends ZIOAppDefault {
  ...
  val run =
    for {
      ...
      _     <- ZIO.foreach(0 to 100)(_ => queue.take.flatMap(i => Console.printLine(s"Got: $i")))
    } yield ()
}
...
object StmPriorityQueue extends ZIOAppDefault {
  ...
  /** EXERCISE
    *
    * Using STM, design a priority queue, where smaller integers are assumed to
    * have higher priority than greater integers.
    */
...
object StmDiningPhilosophers extends ZIOAppDefault {
  ...
  def setupTable(size: Int): ZIO[Any, Nothing, Roundtable] = {
    ...
    (for {
      ...
      placements = (allForks zip allForks.drop(1)).map { case (l, r) =>
                     Placement(l, r)
                   }
